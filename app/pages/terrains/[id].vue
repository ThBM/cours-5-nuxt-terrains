...
const terrainId = useRoute().params.id as string;
...
const { data: existingTerrain } = await useAsyncData(
  `terrain-${terrainId}`,
  async () => {
...
watch(
  ...
  (newVal) => {
    if (newVal) {
      ...
      form.surface = newVal.surface;
      form.surface_constructible = newVal.surface_constructible;
      form.prix = newVal.prix;
      form.longueur_facade = newVal.longueur_facade;
      form.orientation_facade = newVal.orientation_facade;
    }
  },
  { immediate: true },
);
...
<template>
  <UContainer class="py-4">
    ...
    <UCard>
      <template #header>
        <h3 class="font-bold text-xl">Modifier le terrain</h3>
      </template>
